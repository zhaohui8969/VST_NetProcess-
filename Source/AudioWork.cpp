...
// 重采样
std::vector<float> func_audio_resample_vector_version(std::vector<float> inputSampleV, int srcSampleRate, int destSampleRate) {
	if (srcSampleRate == destSampleRate) {
        ...
        std::cout << "Resample " << srcSampleRate << " " << destSampleRate << " " << lInputSize << " " << lOutSize << std::endl;
        auto sinc = new juce::WindowedSincInterpolator();
        sinc->reset();
        sinc->process(fScaleRate, inputSampleV.data(), fReSampleOutBuffer, lOutSize, lInputSize, 0);
		...
		if (queueSize > 0) {
			...
			if (jobStruct.jobType == JOB_EMPTY) {
				...
				if (*bEnableSOVITSPreResample) {
					...
					// SOVITS输入音频重采样
					auto ReSampleOutputVector = func_audio_resample_vector_version(vModelInputSampleBufferVector, dProjectSampleRate, iSOVITSModelInputSamplerate);
					...
					if (*bEnableHUBERTPreResample) {
						// HUBERT输入音频重采样
						auto ReSampleOutputVector = func_audio_resample_vector_version(vModelInputSampleBufferVector, dProjectSampleRate, iHUBERTInputSampleRate);
						long iResampleNumbers = ReSampleOutputVector.size();
				...
				audioFile.setSampleRate(static_cast<uint32_t>(dSOVITSInputSamplerate));
                ...
                std::cout << "请求服务器: " << roleStruct.sApiUrl << endl;

				cli.set_connection_timeout(60, 0); // 300 milliseconds
				...
				*fServerUseTime = tUseTime / 1000.0f;
				...
				if (res.error() == httplib::Error::Success && res->status == 200) {
					...
					// 音频重采样
					currentVoiceVector = func_audio_resample_vector_version(currentVoiceVectorUnResample, sampleRate, dProjectSampleRate);
