...
NetProcessProcessor::NetProcessProcessor ()
	: mBuffer(nullptr)
	, mBufferPos(0)
	, audioFile(sDefaultSaveModelInputWaveFileName)
...
tresult PLUGIN_API NetProcessProcessor::initialize (FUnknown* context)
{
	...
	modelInputAudioBuffer[0].resize(maxOutBufferSize);
...
tresult PLUGIN_API NetProcessProcessor::process (Vst::ProcessData& data)
{
	...
	if (kRecordState == IDLE) {
		...
		if (lModelInputAudioBufferPos + data.numSamples > maxOutBufferSize) {
			bExitWorkState = true;
		...
		if (bExitWorkState) {
			...
			// 保存音频数据到文件
			bool ok = audioFile.setAudioBuffer(modelInputAudioBuffer);
			...
			audioFile.save(sDefaultSaveModelInputWaveFileName, AudioFileFormat::Wave);
			...
			std::ifstream t_pc_file(sDefaultSaveModelInputWaveFileName, std::ios::binary);
			...
			if (res.error() == httplib::Error::Success && res->status == 200) {
				...
				// 写入文件
				std::ofstream t_out_file(sDefaultSaveModelOutputWaveFileName, std::ios::binary);
				...
				tmpAudioFile.load(sDefaultSaveModelOutputWaveFileName);
